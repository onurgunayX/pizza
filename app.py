...
from orderForm import MyForm, myloginform, myregisterform
...
def Register():
    ...
    if request.method=="POST":
        if form.validate_on_submit(): #validate etmeyi yariyor true false
            ...
            new_record = pizza_users_db(
                name_db = form.name.data,
                email_db = form.email.data,
                password_db=bcrypt.generate_password_hash(form.password.data).decode("utf-8"))
    ...
    return render_template("register.html",form=form, name_entered=name_entered,email_entered=email_entered,password_entered=password_entered)
@app.route("/bye")
def bye():
...
def Order():
    ...
    if request.method=="POST":
        ...
        return render_template("successorder.html",form=form, name_entered=name_entered,email_entered=email_entered,many_entered=many_entered,dough_entered=dough_entered, additional_notes_entered=additional_notes_entered,date_entered=date_entered,
            ...
            debit_card_entered=debit_card_entered,created_id=created_id)
...
def update_one(id): #id parametre olarak burada unutulabiliyor!! #11.05.2022
    ...
    if request.method == "POST":
        ...
        record_to_update.many_db=form.many.data
        record_to_update.dough_db = form.dough.data
        record_to_update.additional_notes_db = form.additional_notes.data
        record_to_update.date_db=form.date.data
        record_to_update.time_db=form.time.data
        record_to_update.image_db=form.image.data
        record_to_update.debit_card_db=form.debit_card.data
    ...
    if request.method == "GET":
        ...
        form.many.data= record_to_update.many_db
        form.dough.data=record_to_update.dough_db 
        form.additional_notes.data=record_to_update.additional_notes_db 
        form.date.data=record_to_update.date_db
        form.time.data=record_to_update.time_db
        form.image.data= record_to_update.image_db
        form.debit_card.data=record_to_update.debit_card_db
